#!/usr/bin/env python3
"""Download and validate a royalty-free category stock photo."""
...
def download_url(url: str, dest: Path) -> None:
    ...
    if shutil.which("curl"):
        cmd = ["curl", "-fsSL", "-A", USER_AGENT, "-o", str(dest), url]
...
def is_lfs_pointer(path: Path) -> bool:
    try:
        ...
        return False
    return "git-lfs.github.com/spec/v1" in first_line
...
def validate_image(
    ...
    *,
    ...
    if is_lfs_pointer(path):
        raise ValueError("File is a Git LFS pointer, not an image — run git lfs pull or re-download")
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Download a royalty-free category stock photo and validate dimensions."
    )
    parser.add_argument("--url", required=True, help="Image URL to download")
    parser.add_argument(
        "--output",
        required=True,
        help="Destination path (e.g. public/images/categories/standard-equipment.jpg)",
    )
    parser.add_argument("--min-width", type=int, default=600, help="Minimum width (default: 600)")
    parser.add_argument("--min-height", type=int, default=400, help="Minimum height (default: 400)")
    parser.add_argument("--min-bytes", type=int, default=30_000, help="Minimum file size (default: 30000)")
    ...
    if output.exists() and not args.dry_run and not args.force:
    ...
    try:
        if args.dry_run:
            with tempfile.TemporaryDirectory() as tmp:
                ...
                download_url(args.url, tmp_path)
